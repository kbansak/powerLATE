...
#' @seealso \code{\link{equal.unordered}}, \code{\link{unequal.ordered}}, \code{\link{unequal.unordered}}.
...
equal.ordered <- function(
	...
	if (!is.null(power) && !is.null(N)){
		...
		upper.effect <- (2*M)/sqrt((N*pi^2-4*M^2*(0.5-(pi/2))*(0.5+(pi/2))))
		return(upper.effect)
	}
	...
	if (!is.null(power) && !is.null(kappa)){
		beta <- 1 - power
		M <- qnorm(1-(sig.level/2)) + qnorm(1-beta)
		upper.N <- (4*M^2*(1+kappa^2*(0.5-(pi/2))*(0.5+(pi/2))))/(kappa^2*pi^2)
		return(upper.N)
	}
