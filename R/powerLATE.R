#' @title Power Analysis for Local Average Treatment Effect
#' @aliases powerLATE
...
powerLATE <- function(
	...
	if (sort(lengths(list(N, kappa, power, pi, tau)), decreasing = TRUE)[2]>1){
		stop("Only one in {N, kappa, power, pi, tau} can be a vector")
	}
	if(effect.size){
